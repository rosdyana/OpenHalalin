...
class GeminiService {
  ...
  static Future<void> initialize() async {
    ...
    debugPrint('Gemini model initialized with version: ${AppConfig.geminiVersion}');
  }
    ...
    try {
      ...
      for (final line in text.split('\n')) {
        ...
        if (trimmedLine.startsWith('*')) {
          final content = trimmedLine.substring(trimmedLine.indexOf('*') + 1).trim();
          ...
          } else if (parsingConcerns && !trimmedLine.contains('**EXPLANATIONS:**')) {
      ...
      debugPrint('Finished parsing. Found ${ingredients.length} ingredients and ${concerns.length} concerns');
